...
class MessageHandler():
    def __init__(self):
        self.autopilot_off = rospy.Publisher("/air/autopilot/off", Empty,
                                             queue_size=1)
        self.arm_bridge = rospy.Publisher("/air/bridge/arm", Bool,
                                          queue_size=1)
        self.autopilot_start = rospy.Publisher("/air/autopilot/start", Empty,
                                               queue_size=1)
        self.autopilot_pose_cmd = rospy.Publisher("/air/autopilot/pose_command",
                                                  PoseStamped, queue_size=1)
        self.tree_spacing_cmd = rospy.Publisher("/air/tree_spacing",
                                                Float32, queue_size=1)
        self.obj_spacing_cmd = rospy.Publisher("/air/object_spacing",
                                               Float32, queue_size=1)
        self.reset_exp_pub = rospy.Publisher("/success_reset",
                                             Empty, queue_size=1)
        self.save_pc_pub = rospy.Publisher("/air/save_pc",
                                           Bool, queue_size=1)
